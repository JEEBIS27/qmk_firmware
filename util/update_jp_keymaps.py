...
BASE_NAME = "jp_graphite"

ALT_MAP_RE = re.compile(
    r"static const alt_mapping_t alt_mappings\[\] PROGMEM = \{\n.*?\n\};",
    re.DOTALL,
)
ALT_LANG_RE = re.compile(r"static int alt_lang = \d;\s*// Alternative Layoutの言語設定")
...
def main() -> int:
    ...
    if not base_alt_match:
        raise SystemExit("alt_mappings block not found in jp_graphite")

    for keymap_dir in sorted(KEYMAPS_DIR.iterdir()):
        if not keymap_dir.is_dir():
            continue
        name = keymap_dir.name
        if not name.startswith("jp_") or name == BASE_NAME:
            continue
        ...
        target_alt_lang = ALT_LANG_RE.search(target_text)
        if not target_alt_lang:
            print(f"skip {name}: alt_lang not found")
            continue

        new_text = base_text
        new_text = ALT_MAP_RE.sub(target_alt_match.group(0), new_text, count=1)
        new_text = ALT_LANG_RE.sub(target_alt_lang.group(0), new_text, count=1)
